...
def experiment(
    ...
    states = np.concatenate(states, axis=0) # 拉成了一维向量
    state_mean, state_std = np.mean(states, axis=0), np.std(states, axis=0) + 1e-6

    num_timesteps = sum(traj_lens) # num_timesteps stand for the total trajectories
    ...
    sorted_inds = np.argsort(returns)  # lowest to highestsorted_inds = np.random.randint(0, len(traj_lens) - 1)
    num_trajectories = 1
    timesteps = traj_lens[sorted_inds[-1]] # the timesteps of the highest return trajectory
    ...
    def get_batch(batch_size=256, max_len=200):
        ...
        for i in range(batch_size):
            ...
            si = random.randint(0, traj["observations"].shape[0] - 1)
    ...
    for iter in range(variant["max_iters"]):
        ...
        if log_to_wandb:
            wandb.log(outputs)
    torch.save(model, 'dt_model/2.pt')
